...
public class MahjongSolitaireDataModel extends MiniGameDataModel {
	...
	public void initTiles() {
		...
		String imgPath = props
				.getProperty(MahjongSolitairePropertyType.IMG_PATH);
		int spriteTypeID = 0;
		SpriteType sT;

		// WE'LL RENDER ALL THE TILES ON TOP OF THE BLANK TILE
		String blankTileFileName = props
				.getProperty(MahjongSolitairePropertyType.BLANK_TILE_IMAGE_NAME);
		BufferedImage blankTileImage = miniGame.loadImageWithColorKey(imgPath
				+ blankTileFileName, COLOR_KEY);
		((MahjongSolitairePanel) (miniGame.getCanvas()))
				.setBlankTileImage(blankTileImage);

		// THIS IS A HIGHLIGHTED BLANK TILE FOR WHEN THE PLAYER SELECTS ONE
		String blankTileSelectedFileName = props
				.getProperty(MahjongSolitairePropertyType.BLANK_TILE_SELECTED_IMAGE_NAME);
		...
		String incorrectTileSelectedFileName = props
				.getProperty(MahjongSolitairePropertyType.INCORRECT_BLANK_TILE_SELECTED_IMAGE_NAME);
		BufferedImage incorrectTileSelectedImage = miniGame.loadImageWithColorKey(
				imgPath + incorrectTileSelectedFileName, COLOR_KEY);
		((MahjongSolitairePanel) (miniGame.getCanvas()))
				.setBlockedTileSelectedImage(incorrectTileSelectedImage);

		// FIRST THE TYPE A TILES, OF WHICH THERE IS ONLY ONE OF EACH
		// THIS IS ANALOGOUS TO THE SEASON TILES IN FLAVORLESS MAHJONG
		ArrayList<String> typeATiles = props
				.getPropertyOptionsList(MahjongSolitairePropertyType.TYPE_A_TILES);
		for (int i = 0; i < typeATiles.size(); i++) {
			String imgFile = imgPath + typeATiles.get(i);
			sT = initTileSpriteType(imgFile, TILE_SPRITE_TYPE_PREFIX
					+ spriteTypeID);
		...
		ArrayList<String> typeBTiles = props
				.getPropertyOptionsList(MahjongSolitairePropertyType.TYPE_B_TILES);
		...
		ArrayList<String> typeCTiles = props
				.getPropertyOptionsList(MahjongSolitairePropertyType.TYPE_C_TILES);
	...
	private void initTile(SpriteType sT, String tileType) {
		// CONSTRUCT THE TILE
		MahjongSolitaireTile newTile = new MahjongSolitaireTile(sT,
	...
	// public String tileToText(){return null;};
	...
	private boolean isFreeMove(MahjongSolitaireTile selectTile){
	...
	public MahjongSolitaireMove findMove() {
		...
		for (int i = 0; i < gridColumns; i++) {
			for (int j = 0; j < gridRows; j++) {
				...
				if (stack1.size() > 0) {
					// GET THE FIRST TILE
					MahjongSolitaireTile testTile1 = stack1
							.get(stack1.size() - 1);
					//if(isFreeMove(testTile1))
						...
						for (int l = 0; l < gridRows; l++) {
							if (!((i == k) && (j == l))) {
								...
								if (stack2.size() > 0) {
									...
									MahjongSolitaireTile testTile2 = stack2
											.get(stack2.size() - 1);

									// DO THEY MATCH
									//if(isFreeMove(testTile2))
	...
	public MahjongSolitaireMove getMove(MahjongSolitaireTile testTile1,
			MahjongSolitaireTile testTile2) {
		MahjongSolitaireMove move = new MahjongSolitaireMove();
		move.col1 = testTile1.getGridColumn();
		move.row1 = testTile1.getGridRow();
		move.col2 = testTile2.getGridColumn();
		move.row2 = testTile2.getGridRow();
		return move;

	}

	public boolean blockedTile(MahjongSolitaireTile testTile){
    	MahjongSolitaireMove move = new MahjongSolitaireMove();
    	//move.col1=testTile.getGridColumn();
    	//move.row1=testTile.getGridRow();
    	//move.col2=testTile2.getGridColumn();
    	//move.row2=testTile2.getGridRow();
    	int x=testTile.getGridColumn();
    	int y=testTile.getGridRow();
    	
    	ArrayList<MahjongSolitaireTile> stack1 = tileGrid[x][y];
    	ArrayList<MahjongSolitaireTile> stack2 = tileGrid[x+1][y];
    	ArrayList<MahjongSolitaireTile> stack3 = tileGrid[x-1][y];
	 ...
	 */
	private void moveTiles(ArrayList<MahjongSolitaireTile> from,
			ArrayList<MahjongSolitaireTile> to) {
	...
	public void playWinAnimation() {
		...
		winPath.add(getGameHeight() - WIN_PATH_COORD);*/
		/*for(int w=0;w<50;w++){
		winPath.add(getGameWidth()/2-w);
		winPath.add(WIN_PATH_COORD+w);}
		for(int x=0;x<50;x++){
			winPath.add(getGameWidth()/2-250+x);
			winPath.add(WIN_PATH_COORD+250+x);}
		for(int y=0;y<50;y++){
			winPath.add(getGameWidth()/2+y);
			winPath.add(WIN_PATH_COORD+500-y);}
		for(int z=0;z<50;z++){
			winPath.add(getGameWidth()/2+250-z);
			winPath.add(WIN_PATH_COORD+250-z);}*/
		for(int w=0;w<180;w++){
			winPath.add(getGameWidth()/2+(int)(250*Math.cos(2*w*Math.PI/180)));
			winPath.add(getGameHeight()/2-(int)(250*Math.sin(2*w*Math.PI/180)));}
		//for(int x=0;x<90;x++){
		//	winPath.add(getGameWidth()/2+(int)(250*Math.cos(x*Math.PI/180)));
		//	winPath.add(getGameHeight()/2-(int)(250*Math.sin(x*Math.PI/180)));}
		...
		/*winPath.add(getGameWidth()/2);
		winPath.add(WIN_PATH_COORD);
		winPath.add(getGameWidth()/2);
		winPath.add(WIN_PATH_COORD+500);*/
	...
	public void processMove(MahjongSolitaireMove move) {
		...
		tile2.setTarget(TILE_STACK_X + TILE_STACK_2_OFFSET_X, TILE_STACK_Y
				+ TILE_STACK_OFFSET_Y);
		...
		// PLAY THE AUDIO CUE
		miniGame.getAudio().play(
				MahjongSolitairePropertyType.MATCH_AUDIO_CUE.toString(), false);
		...
		if (stackTiles.size() == NUM_TILES) {
			...
			if (possibleMove == null) {
				...
				//endGameAsLoss();
				miniGame.getGUIDialogs().get(LOSE_DIALOG_TYPE).setState(VISIBLE_STATE);
				miniGame.getAudio().stop(
						MahjongSolitairePropertyType.SPLASH_SCREEN_SONG_CUE.toString());
				miniGame.getAudio().stop(
						MahjongSolitairePropertyType.GAMEPLAY_SONG_CUE.toString());
				miniGame.getAudio().play(
						MahjongSolitairePropertyType.LOSS_AUDIO_CUE.toString(), false);
				endGameAsLoss();
	...
	public void selectTile(MahjongSolitaireTile selectTile) {
		...
		if ((col > 0) && (col < (gridColumns - 1))) {
			...
			if ((z <= leftZ) && (z <= rightZ)) {
				...
				if (selectTile.getState().equals(VISIBLE_STATE)) {
					...
					if(selectedTile!=null&&(selectedTile.getState()==INCORRECTLY_SELECTED_STATE||selectedTile.getState()==SELECTED_STATE))
						selectedTile.setState(VISIBLE_STATE);
					selectedTile = selectTile;
					
					miniGame.getAudio().play(
							MahjongSolitairePropertyType.BLOCKED_TILE_AUDIO_CUE.toString(),
							false);
					return;
				}
			}
		...
		if (selectedTile == null) {
			...
			miniGame.getAudio().play(
					MahjongSolitairePropertyType.SELECT_AUDIO_CUE.toString(),
					false);
		}

		if (selectedTile != selectTile) {
			if(selectedTile.getState()==INCORRECTLY_SELECTED_STATE){
			//	selectTile.setState(SELECTED_STATE);
			//selectedTile.setState(VISIBLE_STATE);
			//selectedTile = selectTile;
			return;}
			if (selectedTile.match(selectTile)) {
				processMove(getMove(selectedTile, selectTile));
				// processMove( new MahjongSolitaireMove)
				//
			}
			...
			// THEY DON'T MATCH, GIVE SOME AUDIO FEEDBACK
				...
				miniGame.getAudio()
						.play(MahjongSolitairePropertyType.NO_MATCH_AUDIO_CUE
								.toString(),
								false);
			}
		}
	...
	public void undoLastMove() {
		if (inProgress() && stackTiles.size() > 1) {
			...
			MahjongSolitaireTile topTile = stackTiles
					.remove(stackTiles.size() - 1);
			MahjongSolitaireTile nextToTopTile = stackTiles.remove(stackTiles
					.size() - 1);
			...
			// PLAY THE AUDIO CUE
			miniGame.getAudio().play(
					MahjongSolitairePropertyType.UNDO_AUDIO_CUE.toString(),
					false);
		}
	}
	...
	public void checkMousePressOnSprites(MiniGame game, int x, int y) {
		...
		if (game.getGUIDialogs().get(STATS_DIALOG_TYPE).getState()
				.equals(VISIBLE_STATE)) {
			game.getGUIDialogs().get(STATS_DIALOG_TYPE)
	...
	public void endGameAsWin() {
		...
		// RECORD IT AS A WIN
		((MahjongSolitaireMiniGame) miniGame).getPlayerRecord().addWin(
		...
		miniGame.getAudio().stop(
				MahjongSolitairePropertyType.GAMEPLAY_SONG_CUE.toString());
		miniGame.getAudio().play(
				MahjongSolitairePropertyType.WIN_AUDIO_CUE.toString(), false);
	}
	...
	public void endGameAsLoss() {
		...
		// RECORD IT AS A WIN
		((MahjongSolitaireMiniGame) miniGame).getPlayerRecord().addLoss(
				currentLevel);
		((MahjongSolitaireMiniGame) miniGame).savePlayerRecord();
	...
	public void reset(MiniGame game) {
		...
		for (int i = 0; i < gridColumns; i++) {
			for (int j = 0; j < gridRows; j++) {
				for (int k = 0; k < levelGrid[i][j]; k++) {
					...
					MahjongSolitaireTile tile = stackTiles.remove(stackTiles
							.size() - 1);
		...
		miniGame.getGUIDialogs().get(LOSE_DIALOG_TYPE)
				.setState(INVISIBLE_STATE);
		miniGame.getGUIDialogs().get(STATS_DIALOG_TYPE)
				.setState(INVISIBLE_STATE);
	}
