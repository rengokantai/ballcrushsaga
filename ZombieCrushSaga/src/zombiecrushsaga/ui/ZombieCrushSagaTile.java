...
public class ZombieCrushSagaTile extends Sprite{
    ...
    public boolean match(ZombieCrushSagaTile testTile)
    {
    	...
    	//UPDATED BY YIDI KE. 10/10/2013
    	if(this.getTileType()==testTile.getTileType()){
    		if(this.getTileType()==TILE_A_TYPE||this.getTileType()==TILE_B_TYPE)
    			return true;
    		if(this.getTileType()==TILE_C_TYPE&&this.spriteType==testTile.spriteType)
    			return true;
    ...
    public void update(MiniGame game)
    {
        ...
        else if (calculateDistanceToTarget() < moveSpeed)
        {
            vX = 0;
            vY = 0;
            x = targetX;
            ...
            if(((ZombieCrushSagaDataModel)((ZombieCrushSagaMiniGame)game).getDataModel()).beginToUpdateBoard){
                if(this.beginExchange){
                 ((ZombieCrushSagaDataModel)((ZombieCrushSagaMiniGame)game).getDataModel()).finishExchange=true;
                 this.beginExchange=false;
                 }
                
                 if(((ZombieCrushSagaDataModel)((ZombieCrushSagaMiniGame)game).getDataModel()).freeMove)
                   ((ZombieCrushSagaDataModel)((ZombieCrushSagaMiniGame)game).getDataModel()).startClear=true;
                 ((ZombieCrushSagaDataModel)((ZombieCrushSagaMiniGame)game).getDataModel()).freeMove=false;
                }
